...
class CNN(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        self.conv = nn.ModuleList(
            [nn.Conv2d(1, self.hparams.out_channels,
                       (kernel, self.embedding.embedding_dim)) for kernel in eval(self.hparams.kernel_heights)
             ])
        self.fc = nn.Linear(len(eval(self.hparams.kernel_heights)) * self.hparams.out_channels, self.n_classes)
    ...
    def forward(self, x, x_len):
        ...
        max_outs = [conv_block(x, conv) for conv in self.conv]

        out = torch.cat(max_outs, 1)
        feat = out
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.opt_list('--out_channels', default=64, type=int, options=[64, 128, 256], tunable=True)
        parser.add_argument('--kernel_heights', default='2, 3, 4, 5', type=str)
...
def conv_block(_input, conv_layer):
    conv_out = conv_layer(_input)                                           # (batch_size, out_channels, dim, 1)
